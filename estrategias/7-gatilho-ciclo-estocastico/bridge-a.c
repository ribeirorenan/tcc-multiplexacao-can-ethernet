...
/*-----------------------------------------------------------
 ...
 /*-----------------------------------------------------------
  ...
  struct timeval tvals_last_receive[MAX_ID + 1];
  ...
  struct timeval tval_received_cycle;
  ...
  while(1){
      ...
      /*
        (FRAMES_PER_BUFFER / 3.0) para uma distribuição normal de probabilidades
        a gente obteve uma media de tres quadros CAN por pacote Ethernet.
          Com uma media de 3 quadros CAN por pacote ethernet, a taxa efetiva de uso do
        barramento é de 62,5%; 1-(14bytes+4bytes)/(3*16bytes)

        Como a quatnidade maxima de quadros CAN de 16 por pactoe ethernet é de
        93 (1500/16), então para maximizar o uso do enlace ethernet nós ajustamos
        a probabilidade em 93/3 (maximo de qudros divido pela média de quadros
        com uma distribuição normal). Com uma media de 42,59 quadros CAN por pacote ethernet, a
        taxa efetiva de uso do barramento é de 97,36%; 1-(14bytes+4bytes)/(42,59*16bytes)

      */
      numero_sorteado = 0 + rand() % 1000;

      if(first_times[frame.can_id] == 1){
        timersub(&tvals_before[frame_count], &tvals_last_receive[frame.can_id], &tval_received_cycle);
        received_cycle = (long int)tval_received_cycle.tv_sec + (long int)tval_received_cycle.tv_usec;
        received_cycle = received_cycle / 1000;

        if(numero_sorteado >= received_cycle * (FRAMES_PER_BUFFER)){
          timer = 0;
        }else{
          timer--;
        }
      ...
      if(timer <= 0){
        ...
        for(int i = 0; i < frame_count; i++){
          ...
          //Getting the las received time to calculate received cycle
          tvals_last_receive[frame_test.can_id] = tvals_before[i];
