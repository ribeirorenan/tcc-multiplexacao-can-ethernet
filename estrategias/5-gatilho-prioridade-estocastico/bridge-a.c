...
/*-----------------------------------------------------------
 ...
 /*-----------------------------------------------------------
  ...
  while(1){
      ...
      */
      numero_sorteado = 0 + rand() % 0x7FF;

      if(numero_sorteado >= frame.can_id * (FRAMES_PER_BUFFER / 3.0)){
        timer = 0;
      }else{
        timer--;
      }
      ...
      /*just printing out the can frame*/
      if(nbytes > 0){
      ...
      if(timer <= 0){
        ...
        for(int i = 0; i < frame_count; i++){
          ...
          if(first_times[frame_test.can_id] == 0){
            ...
            tvals_last_send[frame_test.can_id] = tval_after;
            // printf("%d\t%ld.%06ld\t%ld.%06ld\n", frame_test.can_id, (long int)tval_result.tv_sec, (long int)tval_result.tv_usec, (long int)cycle.tv_sec, (long int)cycle.tv_usec );
            fflush(stdout);
          }
        }
